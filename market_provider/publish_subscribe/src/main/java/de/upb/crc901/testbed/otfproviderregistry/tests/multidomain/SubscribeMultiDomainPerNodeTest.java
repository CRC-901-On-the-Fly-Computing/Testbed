...
/**
 * A static test with multiple domain, where each subscriber randomly subscribes to a subset of the domains.
 * 
 * @author Michael
 *
 */
public class SubscribeMultiDomainPerNodeTest {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			...
			RandomPicker<Domain> picker = new RandomPicker<>(domains);
			for (Domain domain : picker.pickObjects(r.nextInt(domains.size()), true)) {
